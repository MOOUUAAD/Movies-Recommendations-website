  ...
  return (
    <>
        ...
        <div className='h-16 flex items-center'>
            <img src='./assets/logo.png' onClick={() => window.location.reload(false)} className='md:h-14 h-full cursor-pointer object-contain' fill />
        </div>
        ...
        <div className='relative mr-6'>
            <form className=''>                
            <input type="text" placeholder='Enter movie name' onChange={(e)=>{setSearch(e.target.value)}}  onKeyDown={(e) => {searchMovie(e);}}
        ...
        <div className={menu?'bg-[#00172D] absolute top-0 right-0 h-screen w-1/2 z-10':'hidden'}>
          ...
          <h1 className=' m-4 text-3xl mt-20'>Genres</h1>
          <ul className='flex flex-col mt-6 gap-3'>
            {

              arr.map((value,pos)=>{
                  return(
                      <li className='p-2 mx-2 cursor-pointer rounded underline-offset-8 decoration-2 ative:bg-black duration-150 ' onClick={handleMenu}><a href="#" className='focus:text-[#38bdf8]' key={pos} name={value} onClick={(e)=>{getData(e.target.name)}}>{value}</a></li>
                  )
              })
            }
            </ul>
